...
export default class UserLanding extends LightningElement {
    ...
    async connectedCallback() {
        ...
        const userResults = rawResults.filter((game) => game.player_name === this.userName);

        // Place distribution donut
        this.placeDistribution = Object.entries(userResults.reduce((accum, gr) => {
            const {player_num} = gr;
            if (!accum[player_num]) {
                accum[player_num] = 1;
            } else {
            ...
            .map(({name, value}) => { return {name: placeNumberToString[name], value}});
