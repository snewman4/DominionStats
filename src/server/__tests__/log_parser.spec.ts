...
describe('Buy Keyword Handling', () => {
    ...
    it('Valid long card', () => {
        const testCard: PlayedCard[] = handleBuyKeyword(['a', 'Jack', 'of', 'All', 'Trades.']);
    ...
    it('Valid input of strange combination', () => {
        const testCard: PlayedCard[] = handleBuyKeyword(['a', 'Council', 'Room', 'and', 'City', 'Gate.']);

        expect(testCard.length).toEqual(2);
        expect(testCard.filter(element => element.card === 'Council Room').length).toEqual(1);
        expect(testCard.filter(element => element.card === 'City Gate').length).toEqual(1);
...
describe('Play Keyword Handling', () => {
    ...
    it('Valid multiple different cards', () => {
        const testCard: PlayedCard[] = handlePlayKeyword(['2', 'Coppers,', '3', 'Golds', 'and', 'a', 'Silver.']);

        expect(testCard.length).toEqual(6);
        expect(testCard.filter(element => element.card === 'Copper').length).toEqual(2);
        expect(testCard.filter(element => element.card === 'Silver').length).toEqual(1);
        expect(testCard.filter(element => element.card === 'Gold').length).toEqual(3);
        expect(testCard).toContainEqual({
            card: 'Copper',
            effect: [],
            phase: 'buy',
    ...
    it('Valid multiple, single different cards', () => {
        const testCard: PlayedCard[] = handlePlayKeyword(['an', 'Artisan,', 'a', 'Spy', 'and', 'a', 'Gold.']);

        expect(testCard.length).toEqual(3);
        expect(testCard.filter(element => element.card === 'Artisan').length).toEqual(1);
        expect(testCard.filter(element => element.card === 'Spy').length).toEqual(1);
        expect(testCard.filter(element => element.card === 'Gold').length).toEqual(1);
        expect(testCard).toContainEqual({
            card: 'Artisan',
            effect: [],
            phase: 'action',
    ...
    it('Valid strange plurals', () => {
        const testCard: PlayedCard[] = handlePlayKeyword(['2', 'Spies', 'and', '3', 'Rocks.']);

        expect(testCard.length).toEqual(5);
        expect(testCard.filter(element => element.card === 'Spy').length).toEqual(2);
        expect(testCard.filter(element => element.card === 'Rocks').length).toEqual(3);
        expect(testCard).toContainEqual({
            card: 'Spy',
            effect: [],
            phase: 'attack',
    ...
    it('Valid more strange plurals', () => {
        const testCard: PlayedCard[] = handlePlayKeyword(['2', 'Silvers', 'and', '2', 'Platina.']);

        expect(testCard.length).toEqual(4);
        expect(testCard.filter(element => element.card === 'Silver').length).toEqual(2);
        expect(testCard.filter(element => element.card === 'Platinum').length).toEqual(2);
        expect(testCard).toContainEqual({
            card: 'Silver',
            effect: [],
            phase: 'buy',
    ...
    it('Valid long card', () => {
        const testCard: PlayedCard[] = handlePlayKeyword(['a', 'Jack', 'of', 'All', 'Trades.']);
    ...
    it('Valid card using a Way', () => {
        const testCard: PlayedCard[] = handlePlayKeyword(['an', 'Ironmonger', 'using', 'Way', 'of', 'the', 'Monkey.']);
    ...
    it('Valid multi-word card using a Way', () => {
        const testCard: PlayedCard[] = handlePlayKeyword(['a', 'Flag', 'Bearer', 'using', 'Way', 'of', 'the', 'Monkey.']);
